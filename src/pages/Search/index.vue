    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
          ...
          <!-- 分页器 -->
          <Pagination :pageSize="searchParams.pageSize" :pageNo="searchParams.pageNo" :total="searchList.total" :continues="5" @getPageNo="getPageNo"></Pagination>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeMount(){
    ...
    // es6语法：Object.assign，合并对象,相同的属性会合并起来
  ...
  methods: {
    ...
    // 删除分类的名字
    removeCategoryName(){
      // 把带给服务器的数据清空，并且还要再次请求展示默认值
      // 把相应字段置为undefinded，则不会带给服务器
    ...
    removekeyword(){
      ...
      // 通知兄弟组件Header清除关键字
      this.$bus.$emit("clear")
      // 跳转回search组件的默认界面，清除params参数，保留query参数
      if(this.$route.query){
