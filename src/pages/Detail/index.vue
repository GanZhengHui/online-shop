            ...
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"
                  changepirce="0" :class="{ active: spuSaleAttrValue.isChecked == '1' }" @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)" >{{ spuSaleAttrValue.saleAttrValueName
                  }}</dd>
              </dl>
...
export default {
  ...
  methods:{
    ...
    async addShopCar(){
      try {
        const result = await this.$store.dispatch('detail/addOrUpdateShopCar',{
          skuId:this.$route.params.skuid,
          skuNum:this.skuNum})
        // 路由跳转
        // 简单数据由query传递,复杂数据用会话存储(单页面)
