...
//引入lodash中的节流函数，按需引入，throttle是默认暴露
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      try {
        await this.$store.dispatch('detail/addOrUpdateShopCar', { skuId: cart.skuId, skuNum: disNum })
        this.getData()
        console.log('click');
      } catch (error) {
        alert(error.message)
    ...
    async updateChecked(cart, event) {
      ...
      try {
        await this.$store.dispatch('shopCart/reqUpdateCheckedById', { skuId: cart.skuId, isChecked: checked })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
  ...
  computed: {
    ...
    // 是否全部勾选
    isAllCheck() {
      // every,如果每个item中的isChecked都为1，返回true,否则返回fasle
